...
BEGIN
  FOR v_depts IN (SELECT
    ...
    IF v_location_id IS NULL THEN
      RAISE NOTICE 'Record skipped because of invalid location for record: %', v_depts;
      CONTINUE;
    END IF;
  ...
  FOR v_emp IN (SELECT
    ...
    IF v_emp_id IS NULL THEN
      ...
      IF v_emp.ssn IS NOT NULL THEN
        ...
        INSERT INTO employees_audit (
          employee_number, title, first_name, middle_name, last_name, gender, ssn, birth_date,
          marital_status_id, home_email, employment_status_id, hire_date, rehire_date, termination_date,
          term_type_id, term_reason_id, audit_date, audit_action, audit_user
        )
        VALUES (
          v_emp.employee_number, v_emp.title, v_emp.first_name, v_emp.middle_name, v_emp.last_name, v_emp.gender,
          v_emp.ssn, v_emp.birthdate, v_marital_status_id, v_emp.home_email, v_employment_status_id,
          v_emp.orig_hire_date, v_emp.rehire_date, v_emp.term_date, v_term_type_id, v_term_reason_id,
          CURRENT_TIMESTAMP, 'Insert', user
        );
      ELSE
    ...
    IF v_emp_job_id IS NULL THEN
      ...
      INSERT INTO employee_jobs_audit (
        employee_id, job_id, effective_date, expiry_date, pay_amount, standard_hours,
        employee_type_id, employee_status_id, audit_date, audit_action, audit_user
      )
      VALUES (
        v_emp_id, v_job_id, v_emp.job_st_date, v_emp.job_end_date, v_emp.pay_amount,
        v_emp.standard_hours::INT, v_employee_type_id, v_employee_status_id,
        CURRENT_TIMESTAMP, 'Insert', user
      );
    ELSE
      -- UPDATE employee_jobs
...
BEGIN
  FOR v_history IN (
    SELECT
      ...
      e.hire_date AS orig_hire_date,
      e.rehire_date,
      e.termination_date,
      tr.code AS termination_reason_code,
      tr.name AS termination_reason_name,
      tt.code AS termination_type_code,
      ...
      est.code AS employment_status_code,
      est.name AS employment_status_name,
      d.code AS department_code,
      d.name AS department_name,
      l.code AS location_code,
      l.name AS location_name,
      pf.code AS pay_frequency_code,
      pf.name AS pay_frequency_name,
      pt.code AS pay_type_code,
      pt.name AS pay_type_name,
      su.code AS supervisor_job_code,
      su.name AS supervisor_job_title
    FROM employees e
  ...
  VALUES (
    ...
    v_history.gender,
    v_history.ssn,
    v_history.birth_date,
    v_history.marital_status_code,
    v_history.marital_status_name,
    v_history.employee_status_code,
    v_history.employee_status_name,
    v_history.orig_hire_date,
    v_history.rehire_date,
    v_history.termination_date,
    v_history.termination_reason_code,
    v_history.termination_reason_name,
    v_history.termination_type_code,
    v_history.termination_type_name,
    v_history.job_code,
    v_history.job_title,
    v_history.job_st_date,
    v_history.job_end_date,
    v_history.pay_amount,
    v_history.standard_hours,
    v_history.employee_type_code,
    v_history.employee_type_name,
    v_history.employment_status_code,
    v_history.employment_status_name,
    v_history.department_code,
    v_history.department_name,
    v_history.location_code,
    v_history.location_name,
    v_history.pay_frequency_code,
    v_history.pay_frequency_name,
    v_history.pay_type_code,
    v_history.pay_type_name,
    v_history.supervisor_job_code,
    v_history.supervisor_job_title,
    CURRENT_TIMESTAMP
  );
  END LOOP;
END;
...
/* Re-enable triggers once our loads have finished. */
